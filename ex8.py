...
def multiply(f1, f2):
    ...
    for assignment in itertools.product([True, False], repeat=len(vars_new)):
        ...
        p1 = f1.table.get(
            tuple(assign[v] for v in f1.variables), 0
        )
        p2 = f2.table.get(
            tuple(assign[v] for v in f2.variables), 0
        )

        table_new[
            tuple(assign[v] for v in vars_new)
        ] = p1 * p2
...
def marginalize(factor, var):
    ...
    for assignment in itertools.product(
        [True, False], repeat=len(vars_new)
    ):
        prob = 0
...
def variable_elimination(factors, query, evidence, hidden):
    ...
    for var in hidden:
        ...
        if rel:
            ...
            factors = [
                f for f in factors if f not in rel
            ] + [summed]
